...
async function hudlGraphQL({ query, variables, operationName, snifferId }) {
	if (!checkRateLimit(snifferId)) throw new Error('HUDL rate limit exceeded');
	...
	if (!res.ok) throw new Error(`HUDL GraphQL error: ${res.status}`);
	const json = await res.json();
	if (json.errors) throw new Error('HUDL GraphQL: ' + json.errors.map(e => e.message).join(', '));
	return json.data;
}
